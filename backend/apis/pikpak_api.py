...
class PikPakService:
    ...
    async def download_to_root(
        ...
        """
        try:
            result = await client.offline_download(magnet, parent_id="")
            print("=" * 60)
    ...
    async def find_new_folder(
        ...
        while (time.time() - start_time) < timeout:
            try:
                # 获取当前文件夹列表
                current_folder_list = await self.get_folder_list(client)
                current_folder_names = [f["name"] for f in current_folder_list]
                print(f"当前文件夹名称列表: {current_folder_names}")
    ...
    async def batch_download_collection(
        ...
        try:
            # 检查目标文件夹是否已存在
            existing_folders = await self.get_folder_list(client)
            for folder in existing_folders:
            ...
            print(f"下载前的文件夹名称列表: {before_folders}")
                ...
                if result["success"]:
                    ...
                    if new_folder:
                        ...
                        if rename_success:
                            ...
                            # 重命名文件夹内的文件
                            print(
                                "task_id_list ------ batch_download_collection:",
                                task_id_list,
                            )
                            asyncio.create_task(
    ...
    async def get_folder_list(self, client: PikPakApi) -> List[Dict]:
        ...
        Returns:
            ...
            示例: [{"name": "文件夹1", "id": "123"}, {"name": "文件夹2", "id": "456"}]
        """
        ...
        try:
            ...
            file_list = await client.file_list()
            # print("=" * 60)
            # print(f"get_folder_list ---- 文件列表: {file_list}")
            # print("=" * 60)
